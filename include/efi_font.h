/*
 * efi_font.h - 8x16 bitmap font for GOP framebuffer text rendering.
 *
 * CP437-style monospace font covering ASCII 0x20 (space) through 0x7E (~).
 * Each character is 16 bytes (16 rows, 1 byte per row, MSB = leftmost pixel).
 * Derived from the classic VGA 8x16 ROM font (public domain).
 */
...
/*
 * Font data: 95 printable ASCII characters, 16 bytes each = 1520 bytes.
 * Index 0 = space (0x20), index 94 = tilde (0x7E).
 */
static const uint8_t efi_font_data[95][16] = {
...
/* Get a pointer to the glyph for ASCII character `ch`.
   Returns a blank glyph for characters outside 0x20..0x7E. */
static inline const uint8_t *font_glyph(char ch)
{
    int idx = (int)(unsigned char)ch - 0x20;
    if (idx < 0 || idx > 94) {
        return efi_font_data[0]; /* space */
    }
    return efi_font_data[idx];
}
