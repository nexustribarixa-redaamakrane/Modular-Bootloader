...
| **BANcode** | `<bancode/bancode_all.h>` mirrors the upstream generated master header (shared include guard: whichever comes first wins); the surface tracks **BANcode v1.1.0** including the System/App operating-mode API (default System mode — fatal BANcodes dispatch through Kernel Security Traps, the krnl path; App mode delivers them to an app-level crash handler instead). Driver APIs return `bancode_t` codes from the canonical blocks, with trap geometry matching `bancode_to_trap()`. Concrete slots are provisional placements in `<bancode/mbl_bancode.h>`. |
